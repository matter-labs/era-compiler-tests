...
contract Main {
    ...
    function gas_test(uint256 _gas) external {
        ...
        assembly {
            ...
            let result := call(_gas, _gasTest, 0, 0, 0, 32, 32)
    ...
    function address_test(uint256 _address) external {
        assembly {
            let result := call(gas(), _address, 0, 0, 0, 32, 32)
            mstore(0, result)
            return(0, add(32, returndatasize()))
        }
    }
    ...
    function value_test(uint256 value) external payable {
        address _valueTest = address(valueTest);
        assembly {
            let result := call(gas(), _valueTest, value, 0, 0, 32, 32)
            mstore(0, result)
            return(0, add(32, returndatasize()))
        }
    }
    ...
    function input_test(uint256 offset, uint256 len) external {
        ...
        assembly {
            let result := call(gas(), _inputTest, 0, offset, len, 32, 32)
            mstore(0, result)
            return(0, add(32, returndatasize()))
        }
    }
    ...
    function input_test_with_value(uint256 offset, uint256 len, uint256 val) external {
        ...
        assembly {
            ...
            let result := call(gas(), _inputTestWithValue, 0, offset, len, 32, len)
            mstore(0, result)
    ...
    function output_test(uint256 offset, uint256 len, uint256 real_len) external {
        ...
        assembly {
            let result := call(gas(), _outputTest, 0, 0, 0, offset, len)
            mstore(0, result)
    ...
    function output_test_with_value(uint256 offset, uint256 len, uint256 real_len, uint8 val, uint8 before_val) external {
        ...
        assembly {
            ...
            let result := call(gas(), _outputTestWithValue, 0, 0, 0, offset, len)
    ...
    function failure_test() external {
        ...
        assembly {
            let result := call(gas(), _failureTest, 0, 0, 0, 0, 0)
            mstore(0, result)
            return(0, add(32, returndatasize()))
        }
    }
    ...
    function caller_test() external {
        ...
        assembly {
            let result := call(gas(), _callerTest, 0, 0, 0, 32, 32)
            mstore(0, result)
            return(0, add(32, returndatasize()))
        }
    }
    ...
    function callvalue_test() external payable {
        ...
        assembly {
            let result := call(gas(), _callvalueTest, 0, 0, 0, 32, 32)
            mstore(0, result)
            return(0, add(32, returndatasize()))
        }
    }
    ...
    function selfbalance_test() external {
        ...
        assembly {
            let result := call(gas(), _selfbalanceTest, 0, 0, 0, 32, 32)
            mstore(0, result)
            return(0, add(32, returndatasize()))
        }
    }
    ...
    function codesize_test() external {
        ...
        assembly {
            let result := call(gas(), _codesizeTest, 0, 0, 0, 32, 32)
            mstore(0, result)
    ...
    function sstore_test() external {
        ...
        assembly {
            let result := call(gas(), _sstoreTest, 0, 0, 0, 0, 0)
            if or(iszero(result), iszero(eq(returndatasize(), 0))) {
                mstore(0, result)
                returndatacopy(32, 0, returndatasize())
                return(0, add(returndatasize(), 32))
            }
    ...
    function sload_test() external {
        ...
        assembly {
            sstore(0, 0x63)
            let result := call(gas(), _sloadTest, 0, 0, 0, 32, 32)
            mstore(0, result)
            return(0, add(32, returndatasize()))
        }
    }

    function log_test(address _logTest) external {
        assembly {
            let result := call(gas(), _logTest, 0, 0, 0, 0, 0)
            mstore(0, result)
            return(0, add(32, returndatasize()))
        }
    }
