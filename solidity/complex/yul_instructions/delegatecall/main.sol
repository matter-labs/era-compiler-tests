...
contract Main {
    ...
    function gas_test(uint256 _gas) external {
        ...
        assembly {
            ...
            let result := delegatecall(_gas, _gasTest, 0, 0, 32, 32)
    ...
    function address_test(uint256 _address) external {
        assembly {
            let result := delegatecall(gas(), _address, 0, 0, 32, 32)
            mstore(0, result)
            return(0, add(32, returndatasize()))
        }
    }
    ...
    function input_test(uint256 offset, uint256 len) external {
        ...
        assembly {
            let result := delegatecall(gas(), _inputTest, offset, len, 32, 32)
            mstore(0, result)
            return(0, add(32, returndatasize()))
        }
    }
    ...
    function input_test_with_value(uint256 offset, uint256 len, uint256 val) external {
        ...
        assembly {
            ...
            let result := delegatecall(gas(), _inputTestWithValue, offset, len, 32, len)
            mstore(0, result)
    ...
    function output_test(uint256 offset, uint256 len, uint256 real_len) external {
        ...
        assembly {
            sstore(1, real_len)
            let result := delegatecall(gas(), _outputTest, 0, 0, offset, len)
            mstore(0, result)
    ...
    function output_test_with_value(uint256 offset, uint256 len, uint256 real_len, uint8 val, uint8 before_val) external {
        ...
        assembly {
            ...
            let result := delegatecall(gas(), _outputTestWithValue, 0, 0, offset, len)
    ...
    function failure_test() external {
        ...
        assembly {
            let result := delegatecall(gas(), _failureTest, 0, 0, 0, 0)
            mstore(0, result)
            return(0, add(32, returndatasize()))
        }
    }
    ...
    function caller_test() external {
        ...
        assembly {
            let result := delegatecall(gas(), _callerTest, 0, 0, 32, 32)
            mstore(0, result)
            return(0, add(32, returndatasize()))
        }
    }
    ...
    function callvalue_test() external payable {
        ...
        assembly {
            let result := delegatecall(gas(), _callvalueTest, 0, 0, 32, 32)
            mstore(0, result)
            return(0, add(32, returndatasize()))
        }
    }
    ...
    function selfbalance_test() external {
        ...
        assembly {
            let result := delegatecall(gas(), _selfbalanceTest, 0, 0, 32, 32)
            mstore(0, result)
            return(0, add(32, returndatasize()))
        }
    }
    ...
    function codesize_test() external {
        ...
        assembly {
            let result := delegatecall(gas(), _codesizeTest, 0, 0, 32, 32)
            mstore(0, result)
    ...
    function sstore_test() external {
        ...
        assembly {
            let result := delegatecall(gas(), _sstoreTest, 0, 0, 0, 0)
    ...
    function sload_test() external {
        ...
        assembly {
            sstore(0, 0x63)
            let result := delegatecall(gas(), _sloadTest, 0, 0, 32, 32)
            mstore(0, result)
            return(0, add(32, returndatasize()))
        }
    }
    ...
    function log_test() external {
        ...
        assembly {
            let result := delegatecall(gas(), _logTest, 0, 0, 0, 0)
            mstore(0, result)
            return(0, add(32, returndatasize()))
        }
    }
