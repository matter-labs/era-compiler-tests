...
def f() -> Bytes[200]:
    ...
    return Self(self).removeSignature(_abi_encode(0x1234, convert(b"ab", bytes2), convert(x, bytes2), method_id=method_id("g(bytes2,bytes2,bytes2)")))

@external
def f2() -> Bytes[200]:
    x: bytes2 = 0x1234
    return Self(self).removeSignature(_abi_encode(convert(0x1234, uint16), convert(x, uint16), method_id=method_id("h(uint16,uint16)")))
