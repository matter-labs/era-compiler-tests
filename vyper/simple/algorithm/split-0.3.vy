#! { "ignore": true, "modes": [ "V <=0.3.10" ], "cases": [ {
#!     "name": "default",
#!     "inputs": [
#!         {
#!             "method": "f",
...
def split(p: uint256, nodes: Node[3]) -> (uint256, uint256):
    ...
    if nodes[0].key < 1:
        p1: uint256 = 0
        p2: uint256 = 0
        (p1, p2) = staticcall Self(self).split(nodes[p].r, nodes)
        nodes[p].r = p1
        return (p, 0)
    else: # Should be unreachable(nodes[0].key == 0 always), but if remove this branch - test will work.
        p1: uint256 = 0
        p2: uint256 = 0
        (p1, p2) = staticcall Self(self).split(nodes[p].l, nodes)
        nodes[p].l = p2
        return (0, 0)
...
def f() -> Node[3]:
    ...
    r1: uint256 = 0
    r2: uint256 = 0
    (r1, r2) = staticcall Self(self).split(2, nodes)
